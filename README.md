...
- **커서기반 목록조회 구현**:
    - 키워드, 요약에 대한 부분일치 검색 기능 구현
    - `QueryDSL` 기반 동적 검색 조건 처리 기능 구현
- **뉴스기사 삭제 및 복구전략**:
    - 감사 조회에 활용할 수 있는 논리삭제 구현
    - 데이터의 일정기간 보존 후 삭제해야 하는 물리삭제 구현
    - S3에 보관된 백업파일을 탐색해 데이터 복구기능 로직 구현으로 특정날짜의 시스템 장애기능 대응

## Batch 기반 뉴스기사 기능 구현

- **기사 수집 기능 구현**:
    - `@Scheduled`, `JobLauncher`을 사용한 매 시 정각에 자동화
    - Naver의 Json, 조선일보의 RSS의 이기종 소스별 전용파서를 구현해 데이터 표준화 수행
    - 뉴스 링크 기반의 Unique제약으로 뉴스출처별 중복수집을 막고 Naver기사 우선순위 저장정책 채택
- **백업 시스템 및 리소스 최적화**:
    - 발행일 기준 백업 전략으로 뒤늦게 올라올 기사를 대비해 매일 새벽 4시 5분에 작동
    - 백업 도중 시스템 오류로 멈추고 재개할 경우 이전 저장 자료를 append 저장할 수 있는 jsonl형태로 구현
    - S3의 시스템의 저장, 트래픽 리소스를 아끼기 위한 gzip 알고리즘 활용
    - `ArticleBackupJob` 엔티티를 통한 백업 이력 및 상태 로깅 구현

## 코드품질 및 개발 문화 개선
- **AI기반 코드리뷰**:
    - 보안 취약점과 잠재적 런타임 오류를 PR단계에서 조기식별해 버그생성 방지
    - 팀내에서 정한 코드 네이밍 컨벤션 언급기능을 추가해서 코드 베이스 일관성 확보
</details>
